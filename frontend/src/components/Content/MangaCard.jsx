...
const MangaCard = ({ manga, index = 0 }) => {
  ...
  const handleFavoriteClick = (e) => {
    ...
    if (isFavorite(manga.id)) {
      useContentStore.getState().removeFromFavorites(manga.id)
    } else {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-gray-900 rounded-xl overflow-hidden border border-gray-800 hover:border-blue-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/10"
    >
      <Link to={`/manga/${manga.id}`} className="block">
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between">
            <RatingStars rating={manga.rating} size="sm" />
            <span className="text-gray-400 text-xs">{formatDate(manga.releaseDate)}</span>
          </div>
        </div>
      </Link>
    </motion.div>
