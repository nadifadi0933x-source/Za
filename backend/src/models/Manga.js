...
class Manga {
  ...
  create(data) {
    const stmt = db.prepare(`
      INSERT INTO manga (title, title_farsi, description, description_farsi, cover_image, status, rating, author, artist, release_year, chapters_count, tags)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
  ...
  update(id, data) {
    ...
    const sql = `UPDATE manga SET ${updates.join(', ')} WHERE id = ?`;
    const stmt = db.prepare(sql);
    stmt.run(...params);
    return this.findById(id);
  }
  ...
  getAverageRating(mangaId) {
    const stmt = db.prepare('SELECT AVG(rating) as avg_rating, COUNT(*) as review_count FROM reviews WHERE content_type = "manga" AND content_id = ?');
